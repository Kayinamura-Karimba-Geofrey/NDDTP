...
export const SERVICE_PORTS: Record<ServiceKey, number> = Object.fromEntries(
  Object.entries(MICROSERVICES).map(([key, { port }]) => [key, port]),
) as Record<ServiceKey, number>;

/** Dev proxy prefix: /api/svc/{key}/... → localhost:{port}/api/v1/... */
export const API_SERVICE_BASE = '/api/svc';

export function serviceBaseUrl(key: ServiceKey): string {
  return `${API_SERVICE_BASE}/${key}`;
}
