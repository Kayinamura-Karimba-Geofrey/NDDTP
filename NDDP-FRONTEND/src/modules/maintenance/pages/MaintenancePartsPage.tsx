...
import { MOCK_PARTS, type MaintenancePart } from '../constants/maintenance-data';
...
export function MaintenancePartsPage() {
  ...
  return (
    <div>
      <PageHeader breadcrumbs={[{ label: 'Maintenance', path: '/maintenance/dashboard' }, { label: 'Parts' }]} title="Parts Inventory" description="Spare parts stock levels tied to maintenance work orders" actions={<Button onClick={() => toast('Add part')}><FiPlus className="h-4 w-4" /> Add Part</Button>} />
      <MaintenanceSubNav />
      <Card><CardContent className="pt-6"><DataTable columns={columns as unknown as DataTableColumn<Record<string, unknown>>[]} rows={MOCK_PARTS as unknown as Record<string, unknown>[]} rowKey={(r) => String(r.id)} /></CardContent></Card>
    </div>
  );
}
