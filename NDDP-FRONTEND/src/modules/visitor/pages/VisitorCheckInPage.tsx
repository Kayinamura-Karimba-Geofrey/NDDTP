...
export function VisitorCheckInPage() {
  ...
  return (
    <div>
      <PageHeader breadcrumbs={[{ label: 'Visitors', path: '/visitors/dashboard' }, { label: 'Check-In' }]} title="Check-In Desk" description="Record visitor check-in and check-out at controlled sites" />
      <VisitorSubNav />
      <Card className="mb-6">
        <CardContent className="grid gap-4 pt-6 sm:grid-cols-3">
          ...
          <div className="sm:col-span-3 flex gap-2">
            <Button onClick={() => toast('Checked in')}>Check In</Button>
            <Button variant="outline" onClick={() => toast('Checked out')}>Check Out</Button>
          </div>
        </CardContent>
      </Card>
