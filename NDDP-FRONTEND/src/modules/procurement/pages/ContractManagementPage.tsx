...
import { MOCK_CONTRACTS, type ContractRecord } from '../constants/procurement-data';
...
export function ContractManagementPage() {
  ...
  return (
    <div>
      <PageHeader breadcrumbs={[{ label: 'Procurement', path: '/procurement/dashboard' }, { label: 'Contracts' }]} title="Contract Management" description="Procurement contracts — renewals, amendments, milestones" actions={<Button onClick={() => toast('New contract')}><FiPlus className="h-4 w-4" /> New Contract</Button>} />
      <ProcurementSubNav />
      <Card>
        <CardContent className="pt-6">
          <DataTable columns={columns as unknown as DataTableColumn<Record<string, unknown>>[]} rows={MOCK_CONTRACTS as unknown as Record<string, unknown>[]} rowKey={(r) => String(r.id)} />
        </CardContent>
      </Card>
    </div>
  );
