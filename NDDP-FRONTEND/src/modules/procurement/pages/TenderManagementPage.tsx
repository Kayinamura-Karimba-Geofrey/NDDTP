...
import { MOCK_TENDERS, type TenderRecord } from '../constants/procurement-data';
...
export function TenderManagementPage() {
  ...
  return (
    <div>
      <PageHeader breadcrumbs={[{ label: 'Procurement', path: '/procurement/dashboard' }, { label: 'Tenders' }]} title="Tender Management" description="High-value procurements — publish, evaluate, award" actions={<Button onClick={() => toast('Publish tender')}><FiPlus className="h-4 w-4" /> Publish Tender</Button>} />
      <ProcurementSubNav />
      <Card>
        <CardContent className="pt-6">
          <DataTable columns={columns as unknown as DataTableColumn<Record<string, unknown>>[]} rows={MOCK_TENDERS as unknown as Record<string, unknown>[]} rowKey={(r) => String(r.id)} />
        </CardContent>
      </Card>
    </div>
  );
