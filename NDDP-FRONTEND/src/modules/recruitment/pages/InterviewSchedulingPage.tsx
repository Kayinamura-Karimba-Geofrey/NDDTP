...
export function InterviewSchedulingPage() {
  ...
  const columns: DataTableColumn<Interview>[] = [
    ...
    { key: 'actions', header: 'Actions', render: () => <Button variant="ghost" size="sm" onClick={() => toast('Reschedule')}>Reschedule</Button> },
  ];

  return (
    <div>
      <PageHeader breadcrumbs={[{ label: 'Recruitment', path: '/recruitment/dashboard' }, { label: 'Interview Scheduling' }]} title="Interview Scheduling" description="Calendar-based interview scheduling — daily, weekly, monthly views" actions={<Button onClick={() => toast('Schedule interview')}><FiPlus className="h-4 w-4" /> Schedule Interview</Button>} />
      <RecruitmentSubNav />
