...
export function WorkforceRequestsPage() {
  const columns: DataTableColumn<WorkforceRequest>[] = [
    ...
    { key: 'actions', header: 'Actions', render: () => <Button variant="ghost" size="sm" onClick={() => toast('View workflow')}>Workflow</Button> },
  ];

  return (
    <div>
      <PageHeader breadcrumbs={[{ label: 'Recruitment', path: '/recruitment/dashboard' }, { label: 'Workforce Requests' }]} title="Workforce Requests" description="Department staffing requests before recruitment begins" actions={<Button onClick={() => toast('New request')}><FiPlus className="h-4 w-4" /> New Request</Button>} />
      <RecruitmentSubNav />
