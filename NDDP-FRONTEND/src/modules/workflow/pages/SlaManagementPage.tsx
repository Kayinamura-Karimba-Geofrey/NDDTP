...
import { useGetSlaRulesQuery } from '../api/workflow.api';
import type { SlaRule } from '../constants/workflow-data';
import { CreateSlaRuleModal } from '../components/CreateSlaRuleModal';

export function SlaManagementPage() {
  const { data: slas = [], isLoading } = useGetSlaRulesQuery();
  ...
  return (
    <div>
      <PageHeader breadcrumbs={[{ label: 'Workflow', path: '/workflow/dashboard' }, { label: 'SLA' }]} title="SLA Management" description="Deadline targets per workflow type with compliance tracking" actions={<Button onClick={() => setIsModalOpen(true)}><FiPlus className="h-4 w-4" /> Add SLA Target</Button>} />
      <WorkflowSubNav />
      ...
      <Card><CardContent className="pt-6">
        {isLoading ? <div className="data-table-empty">Loading...</div> : (
          <DataTable columns={columns as unknown as DataTableColumn<Record<string, unknown>>[]} rows={slas as unknown as Record<string, unknown>[]} rowKey={(r) => String(r.id)} />
        )}
      </CardContent></Card>
