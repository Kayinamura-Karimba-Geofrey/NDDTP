...
import { MOCK_KPIS, type KpiRecord } from '../constants/performance-data';
...
export function KpiManagementPage() {
  ...
  return (
    <div>
      <PageHeader breadcrumbs={[{ label: 'Performance', path: '/performance/dashboard' }, { label: 'KPIs' }]} title="KPI Management" description="Measurable key performance indicators" actions={<Button onClick={() => toast('Add KPI')}><FiPlus className="h-4 w-4" /> Add KPI</Button>} />
      <PerformanceSubNav />
      <Card><CardContent className="pt-6"><DataTable columns={columns as unknown as DataTableColumn<Record<string, unknown>>[]} rows={MOCK_KPIS as unknown as Record<string, unknown>[]} rowKey={(r) => String(r.id)} /></CardContent></Card>
    </div>
  );
}
