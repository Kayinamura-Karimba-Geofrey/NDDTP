...
export default function extractCountryCallingCodeFromInternationalNumberWithoutPlusSign(number, country, defaultCountry, defaultCallingCode, metadataJson) {
  ...
  const countryCallingCode = country || defaultCountry ? getCountryCallingCode(country || defaultCountry, metadataJson) : defaultCallingCode;
  if (number.indexOf(countryCallingCode) === 0) {
    ...
    } = extractNationalNumber(possibleShorterNumber, undefined, metadata);
    const {
      nationalNumber
    } = extractNationalNumber(number, undefined, metadata);
