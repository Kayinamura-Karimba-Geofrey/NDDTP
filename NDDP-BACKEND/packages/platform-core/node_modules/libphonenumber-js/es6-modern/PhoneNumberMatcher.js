...
export default class PhoneNumberMatcher {
    ...
    for (const innerMatchRegExp of this.INNER_MATCHES) {
      ...
      while (this.maxTries > 0 && (candidateMatch = innerMatchRegExp.exec(substring)) !== null) {
        if (isFirstMatch) {
          ...
          const _candidate = trimAfterFirstMatch(UNWANTED_END_CHAR_PATTERN, substring.slice(0, candidateMatch.index));
          const _match = this.parseAndVerify(_candidate, offset, text);
          if (_match) {
            return _match;
          }
          this.maxTries--;
  ...
  parseAndVerify(candidate, offset, text) {
    ...
    const phoneNumber = parsePhoneNumber(candidate, {
      extended: true,
