...
const VALID_FORMATTED_PHONE_NUMBER_DIGITS_PART = '[' + VALID_PUNCTUATION + VALID_DIGITS + ']+';
const VALID_FORMATTED_PHONE_NUMBER_DIGITS_PART_PATTERN = new RegExp('^' + VALID_FORMATTED_PHONE_NUMBER_DIGITS_PART + '$', 'i');
const VALID_FORMATTED_PHONE_NUMBER_PART = '(?:' + '[' + PLUS_CHARS + ']' + '[' + VALID_PUNCTUATION + VALID_DIGITS + ']*' + '|' + '[' + VALID_PUNCTUATION + VALID_DIGITS + ']+' + ')';
const AFTER_PHONE_NUMBER_DIGITS_END_PATTERN = new RegExp('[^' + VALID_PUNCTUATION + VALID_DIGITS + ']+' + '.*' + '$');
...
export default class AsYouTypeParser {
  ...
  inputDigits(nextDigits, state) {
    ...
    if (!state.international) {
      if (!this.hasExtractedNationalSignificantNumber) {
        this.extractNationalSignificantNumber(state.getNationalDigits(), stateUpdate => state.update(stateUpdate));
      }
    }
  }
  ...
  extractCountryCallingCode(state) {
    const {
      countryCallingCode,
      number
    } = extractCountryCallingCode('+' + state.getDigitsWithoutInternationalPrefix(), state.country, this.defaultCountry, this.defaultCallingCode, this.metadata.metadata);
  ...
  extractNationalSignificantNumber(nationalDigits, setState) {
    ...
    const {
      ...
      nationalNumber,
      carrierCode
    } = extractNationalNumberFromPossiblyIncompleteNumber(nationalDigits, this.metadata);
  ...
  onExtractedNationalNumber(nationalPrefix, carrierCode, nationalSignificantNumber, nationalDigits, setState) {
  ...
  reExtractNationalSignificantNumber(state) {
    ...
    //
    if (this.extractAnotherNationalSignificantNumber(state.getNationalDigits(), state.nationalSignificantNumber, stateUpdate => state.update(stateUpdate))) {
      return true;
    }
  ...
  extractIddPrefix(state) {
    ...
    // https://github.com/catamphetamine/libphonenumber-js/issues/185
    // Detect such numbers.
    const numberWithoutIDD = stripIddPrefix(digits, this.defaultCountry, this.defaultCallingCode, this.metadata.metadata);
  ...
  fixMissingPlus(state) {
    if (!state.international) {
      ...
      } = extractCountryCallingCodeFromInternationalNumberWithoutPlusSign(state.digits, state.country, this.defaultCountry, this.defaultCallingCode, this.metadata.metadata);
      if (newCallingCode) {
        state.update({
          missingPlus: true
        });
