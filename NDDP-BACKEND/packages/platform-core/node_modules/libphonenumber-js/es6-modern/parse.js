...
export default function parse(text, options, metadataJson) {
  ...
  const {
    ...
    ext,
    error
  } = parseInput(text, options.v2, options.extract);
  ...
  const {
    ...
    carrierCode
  } = parsePhoneNumber(formattedPhoneNumber, options.defaultCountry, options.defaultCallingCode,
  ...
  if (options.v2) {
    const phoneNumber = new PhoneNumber(countryCallingCode, nationalNumber, metadata.metadata);
    if (country) {
...
function parsePhoneNumber(formattedPhoneNumber, defaultCountry, defaultCallingCode, metadata) {
  ...
  let {
    ...
    number
  } = extractCountryCallingCode(parseIncompletePhoneNumber(formattedPhoneNumber), undefined,
  ...
  const {
    nationalNumber,
    carrierCode
  } = extractNationalNumber(parseIncompletePhoneNumber(number), undefined, metadata);
