...
// `libphonenumber` code.
//
const NON_ALTERING_FORMAT_REG_EXP = new RegExp('[' + VALID_PUNCTUATION + ']*' +
...
export default class AsYouTypeFormatter {
  ...
  format(nextDigits, state) {
    ...
    if (canFormatCompleteNumber(state.nationalSignificantNumber, this.metadata)) {
      for (const format of this.matchingFormats) {
        const formattedCompleteNumber = formatCompleteNumber(state, format, {
  ...
  narrowDownMatchingFormats({
    ...
    this.matchingFormats = this.matchingFormats.filter(format => this.formatSuits(format, international, nationalPrefix) && this.formatMatches(format, leadingDigits, leadingDigitsPatternIndex));
  ...
  getTemplate(state) {
    ...
    let index = -1;
    let i = 0;
    const internationalPrefix = state.international ? this.getInternationalPrefixBeforeCountryCallingCode(state, {
      spacing: false
    }) : '';
  ...
  getTemplateForFormat(format, {
    ...
    if (this.shouldTryNationalPrefixFormattingRule(format, {
      ...
      const numberFormatWithNationalPrefix = numberFormat.replace(FIRST_GROUP_PATTERN, format.nationalPrefixFormattingRule());
  ...
  formatNextNationalNumberDigits(digits) {
    const result = populateTemplateWithDigits(this.populatedNationalNumberTemplate, this.populatedNationalNumberTemplatePosition, digits);
    if (!result) {
      // Reset the format.
      this.resetFormat();
      return;
  ...
  shouldTryNationalPrefixFormattingRule(format, {
    international,
    nationalPrefix
  }) {
