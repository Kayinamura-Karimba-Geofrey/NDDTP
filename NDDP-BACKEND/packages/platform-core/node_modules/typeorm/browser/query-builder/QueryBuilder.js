    ...
    *getPredicates(where) {
        ...
        for (const [key, value] of Object.entries(sort)) {
            if (typeof value === "string") {
                if (!validOrders.includes(value))
                    throw new TypeORMError(`Invalid order direction "${value}" for "${key}". Allowed values: ${validOrders.join(", ")}.`);
            }
            else if (typeof value === "object" && value !== null) {
                if (!validOrders.includes(value.order))
                    throw new TypeORMError(`Invalid order direction "${value.order}" for "${key}". Allowed values: ${validOrders.join(", ")}.`);
                if (value.nulls !== undefined &&
                    !validNulls.includes(value.nulls))
                    throw new TypeORMError(`Invalid nulls option "${value.nulls}" for "${key}". Allowed values: ${validNulls.join(", ")}.`);
            }
            else {
                throw new TypeORMError(`Invalid order-by value for "${key}". Expected "ASC", "DESC", or { order, nulls } object.`);
            }
        }
    }
    normalizeNumber(num) {
    ...
    validateNumericInput(label, num) {
        ...
        if (normalized !== undefined && isNaN(normalized))
            throw new TypeORMError(`Provided "${label}" value is not a number. Please provide a numeric value.`);
        return normalized;
    }
}
/**
