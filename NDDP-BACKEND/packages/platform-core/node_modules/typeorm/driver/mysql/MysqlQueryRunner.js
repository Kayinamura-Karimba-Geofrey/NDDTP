...
class MysqlQueryRunner extends BaseQueryRunner_1.BaseQueryRunner {
    ...
    async loadTables(tableNames) {
        ...
        return Promise.all(dbTables.map(async (dbTable) => {
            ...
            table.columns = await Promise.all(dbColumns
                ...
                if (this.driver.unsignedColumnTypes.includes(tableColumn.type)) {
                ...
                if (this.driver.withLengthColumnTypes.indexOf(tableColumn.type) !== -1 &&
                    ...
                    let length = dbColumn["CHARACTER_MAXIMUM_LENGTH"];
                    if (tableColumn.type === "vector") {
                        // MySQL and MariaDb store the vector length in bytes, not in number of dimensions.
                        length = length / 4;
                    }
                    tableColumn.length =
                        !this.isDefaultColumnLength(table, tableColumn, length.toString())
                            ? length.toString()
                            : "";
                }
    ...
    isDefaultColumnWidth(table, column, width) {
        ...
        if (this.driver.options.type === "mysql" &&
            DriverUtils_1.DriverUtils.isReleaseVersionOrGreater(this.driver, "8.0.0")) {
            return true;
        }
