...
export const createAppConfig = (defaults: PlatformServiceDefaults) =>
  registerAs('app', () => ({
...
export const createRedisConfig = (defaultDb: number) =>
  registerAs('redis', () => ({
...
export const createRabbitmqConfig = (queuePrefix: string) =>
  registerAs('rabbitmq', () => ({
...
export const createPlatformConfiguration = (defaults: PlatformServiceDefaults) => [
  createAppConfig(defaults),
  registerAs('database', () => ({
    host: process.env.DB_HOST || 'localhost',
    port: parseInt(process.env.DB_PORT || '5432', 10),
  ...
  createRedisConfig(defaults.redisDb),
  createRabbitmqConfig(defaults.queuePrefix),
  jwtConfig,
  securityConfig,
];
