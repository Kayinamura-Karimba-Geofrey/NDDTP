...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3023;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Calendar Service')
    .setDescription('Calendars, events, attendees, and RSVP management')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Calendars').addTag('Calendar Events').addTag('Event Attendees')
    .addTag('Health')
    .build();
  ...
  Logger.log(`Calendar Service on port ${port}`, 'Bootstrap');
