...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3008;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Leave Service')
    .setDescription('Leave requests, balances, approvals, and accrual management')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Leave Types').addTag('Leave Balances').addTag('Leave Requests').addTag('Health')
    .build();
  ...
  Logger.log(`Leave Service on port ${port}`, 'Bootstrap');
