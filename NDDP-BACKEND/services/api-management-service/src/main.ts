...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3032;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP API Management Service')
    .setDescription('API product catalog, route mappings, and consumer key management')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('API Products').addTag('API Routes').addTag('API Consumers')
    .addTag('Health')
    .build();
  ...
  Logger.log(`API Management Service on port ${port}`, 'Bootstrap');
