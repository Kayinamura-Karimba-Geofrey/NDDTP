...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3033;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Backup Service')
    .setDescription('Backup policies, job execution, and restore operations')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Backup Policies').addTag('Backup Jobs').addTag('Backup Restores')
    .addTag('Health')
    .build();
  ...
  Logger.log(`Backup Service on port ${port}`, 'Bootstrap');
