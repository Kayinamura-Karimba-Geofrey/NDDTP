...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3031;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Integration Service')
    .setDescription('External system connectors, endpoint mappings, and sync job execution')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Connectors').addTag('Endpoints').addTag('Integration Jobs')
    .addTag('Health')
    .build();
  ...
  Logger.log(`Integration Service on port ${port}`, 'Bootstrap');
