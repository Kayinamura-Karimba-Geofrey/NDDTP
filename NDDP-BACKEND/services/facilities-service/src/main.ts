...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3019;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Facilities Service')
    .setDescription('Facility types, facilities, spaces, and booking management')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Facility Types').addTag('Facilities').addTag('Facility Spaces')
    .addTag('Space Bookings').addTag('Health')
    .build();
  ...
  Logger.log(`Facilities Service on port ${port}`, 'Bootstrap');
