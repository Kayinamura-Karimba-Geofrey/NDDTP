...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3034;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Monitoring Service')
    .setDescription('Monitoring targets, health checks, and alert management')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Monitoring Targets').addTag('Health Checks').addTag('Monitoring Alerts')
    .addTag('Health')
    .build();
  ...
  Logger.log(`Monitoring Service on port ${port}`, 'Bootstrap');
