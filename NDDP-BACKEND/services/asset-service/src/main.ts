...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3013;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Asset Service')
    .setDescription('Asset categories, registry, assignments, movements, and audits')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Asset Categories').addTag('Assets').addTag('Asset Assignments')
    .addTag('Asset Movements').addTag('Asset Audits').addTag('Health')
    .build();
  ...
  Logger.log(`Asset Service on port ${port}`, 'Bootstrap');
