...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3025;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Analytics Service')
    .setDescription('Metric definitions, snapshots, dashboards, and analytics queries')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Metrics').addTag('Dashboards').addTag('Analytics Queries')
    .addTag('Health')
    .build();
  ...
  Logger.log(`Analytics Service on port ${port}`, 'Bootstrap');
