...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3009;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Welfare Service')
    .setDescription('Welfare programs, dependents, claims, and disbursements')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Welfare Programs').addTag('Dependents').addTag('Welfare Claims').addTag('Health')
    .build();
  ...
  Logger.log(`Welfare Service on port ${port}`, 'Bootstrap');
