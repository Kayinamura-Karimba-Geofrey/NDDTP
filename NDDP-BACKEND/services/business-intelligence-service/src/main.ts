...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3026;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Business Intelligence Service')
    .setDescription('BI datasets, semantic models, report definitions, and executions')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Datasets').addTag('Semantic Models').addTag('BI Reports')
    .addTag('Health')
    .build();
  ...
  Logger.log(`Business Intelligence Service on port ${port}`, 'Bootstrap');
