...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3021;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Visitor Service')
    .setDescription('Visit sites, visitor registration, visit requests, and check-in management')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Visit Sites').addTag('Visitors').addTag('Visit Requests')
    .addTag('Check-ins').addTag('Health')
    .build();
  ...
  Logger.log(`Visitor Service on port ${port}`, 'Bootstrap');
