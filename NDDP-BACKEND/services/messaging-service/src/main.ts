...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3027;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Messaging Service')
    .setDescription('Channels, members, messages, and delivery receipts')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Channels').addTag('Messages')
    .addTag('Health')
    .build();
  ...
  Logger.log(`Messaging Service on port ${port}`, 'Bootstrap');
