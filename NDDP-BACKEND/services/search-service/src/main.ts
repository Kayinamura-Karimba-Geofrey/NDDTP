...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3029;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Search Service')
    .setDescription('Search indexes, document indexing, and query execution')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Search Indexes').addTag('Search Documents').addTag('Search Queries')
    .addTag('Health')
    .build();
  ...
  Logger.log(`Search Service on port ${port}`, 'Bootstrap');
