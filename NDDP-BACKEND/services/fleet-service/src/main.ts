...
async function bootstrap(): Promise<void> {
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Fleet Service')
    .setDescription('Vehicle types, fleet vehicles, assignments, trips, and inspections')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Vehicle Types').addTag('Vehicles').addTag('Vehicle Assignments')
    .addTag('Trip Logs').addTag('Vehicle Inspections').addTag('Health')
    .build();
  ...
  Logger.log(`Fleet Service on port ${port}`, 'Bootstrap');
