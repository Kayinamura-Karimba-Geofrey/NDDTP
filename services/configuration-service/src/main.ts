...
async function bootstrap(): Promise<void> {
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Configuration Service')
    .setDescription('Configuration namespaces, entries, and revision history')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Namespaces').addTag('Config Entries').addTag('Revisions')
    .addTag('Health')
    .build();
  ...
  Logger.log(`Configuration Service on port ${port}`, 'Bootstrap');
