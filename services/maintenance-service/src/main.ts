...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3018;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Maintenance Service')
    .setDescription('Categories, requests, work orders, tasks, and maintenance logs')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Maintenance Categories').addTag('Maintenance Requests').addTag('Work Orders')
    .addTag('Work Order Tasks').addTag('Health')
    .build();
  ...
  Logger.log(`Maintenance Service on port ${port}`, 'Bootstrap');
