...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3003;
  ...
  const swagger = new DocumentBuilder()
    ...
    .setDescription('User profiles, departments, addresses, and onboarding for NDDTP')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Users').addTag('Departments').addTag('Health')
    .build();
  ...
  Logger.log(`User Service running on port ${port}`, 'Bootstrap');
