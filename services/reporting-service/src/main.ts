...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3024;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Reporting Service')
    .setDescription('Report definitions, generation requests, schedules, and outputs')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Report Definitions').addTag('Report Schedules').addTag('Report Requests')
    .addTag('Health')
    .build();
  ...
  Logger.log(`Reporting Service on port ${port}`, 'Bootstrap');
