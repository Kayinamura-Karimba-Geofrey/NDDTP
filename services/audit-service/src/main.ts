...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3005;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Audit Service')
    .setDescription('Centralized compliance audit logging, security events, and immutable audit trail')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Audit Logs').addTag('Security Events').addTag('Health')
    .build();
  ...
  Logger.log(`Audit Service on port ${port}`, 'Bootstrap');
