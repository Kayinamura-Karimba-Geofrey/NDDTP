...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3016;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Procurement Service')
    .setDescription('Vendors, purchase requisitions, orders, and goods receipts')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Vendors').addTag('Purchase Requisitions').addTag('Purchase Orders')
    .addTag('Goods Receipts').addTag('Health')
    .build();
  ...
  Logger.log(`Procurement Service on port ${port}`, 'Bootstrap');
