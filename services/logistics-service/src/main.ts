...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3015;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Logistics Service')
    .setDescription('Locations, transport routes, shipments, and tracking')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Locations').addTag('Transport Routes').addTag('Shipments')
    .addTag('Shipment Tracking').addTag('Health')
    .build();
  ...
  Logger.log(`Logistics Service on port ${port}`, 'Bootstrap');
