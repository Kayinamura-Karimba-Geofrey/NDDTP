...
export class InvalidStatusTransitionException extends HttpException {
  constructor(from: string, to: string) {
    super({ statusCode: HttpStatus.BAD_REQUEST, message: `Cannot transition message from ${from} to ${to}`, error: 'Bad Request' }, HttpStatus.BAD_REQUEST);
  }
}
...
export class ForbiddenChannelAccessException extends HttpException {
  constructor() {
    super({ statusCode: HttpStatus.FORBIDDEN, message: 'You are not a member of this channel', error: 'Forbidden' }, HttpStatus.FORBIDDEN);
  }
}
