...
export class InvalidStatusTransitionException extends HttpException {
  constructor(from: string, to: string) {
    super({ statusCode: HttpStatus.BAD_REQUEST, message: `Cannot transition from ${from} to ${to}`, error: 'Bad Request' }, HttpStatus.BAD_REQUEST);
  }
}
...
export class ForbiddenConversationAccessException extends HttpException {
  constructor() {
    super({ statusCode: HttpStatus.FORBIDDEN, message: 'You do not have access to this conversation', error: 'Forbidden' }, HttpStatus.FORBIDDEN);
  }
}
