...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3035;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP AI Assistant Service')
    .setDescription('AI agents, conversations, and message management')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('AI Agents').addTag('AI Conversations').addTag('AI Messages')
    .addTag('Health')
    .build();
  ...
  Logger.log(`AI Assistant Service on port ${port}`, 'Bootstrap');
