...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3028;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Announcement Service')
    .setDescription('Announcement categories, publishing workflow, and acknowledgements')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Categories').addTag('Announcements').addTag('Acknowledgements')
    .addTag('Health')
    .build();
  ...
  Logger.log(`Announcement Service on port ${port}`, 'Bootstrap');
