...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3014;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Inventory Service')
    .setDescription('Warehouses, items, stock levels, movements, and requisitions')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Warehouses').addTag('Inventory Items').addTag('Stock Levels')
    .addTag('Stock Requests').addTag('Health')
    .build();
  ...
  Logger.log(`Inventory Service on port ${port}`, 'Bootstrap');
