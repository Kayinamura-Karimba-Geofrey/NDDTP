...
async function bootstrap(): Promise<void> {
  ...
  const port = cs.get<number>('app.port') || 3007;
  ...
  const swagger = new DocumentBuilder()
    .setTitle('NDDTP Recruitment Service')
    .setDescription('Job postings, candidate applications, interviews, and hiring pipeline')
    .setVersion('1.0')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'access-token')
    .addTag('Job Postings').addTag('Applications').addTag('Interviews').addTag('Health')
    .build();
  ...
  Logger.log(`Recruitment Service on port ${port}`, 'Bootstrap');
